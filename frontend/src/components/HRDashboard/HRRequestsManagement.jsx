                ...
                <div className="flex items-center gap-1.5 bg-slate-100 dark:bg-slate-900/50 p-1 rounded-2xl border border-slate-200/60 dark:border-slate-700/50 self-start sm:self-auto">
                    <button
                        ...
                        className={`flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-bold transition-all cursor-pointer ${subTab === 'general'
                                ? 'bg-white dark:bg-slate-800 text-indigo-600 dark:text-indigo-400 shadow-xs border border-slate-200/60 dark:border-slate-600/50'
                    ...
                    <div className="flex flex-wrap items-center gap-3">
                        <div className="relative flex-1 min-w-[200px]">
                            ...
                            <input
                                ...
                                className="w-full pl-9 pr-4 py-2.5 bg-white dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700/50 rounded-xl text-xs font-semibold text-slate-700 dark:text-slate-200 outline-none focus:border-indigo-400 transition-all placeholder:text-slate-300 dark:placeholder:text-slate-500"
                            />
                        </div>
                                ...
                                return (
                                    <button
                                        ...
                                        className={`text-xs font-bold px-3 py-2 rounded-xl border transition-all cursor-pointer ${statusFilter === s
                                                ? 'bg-indigo-50 dark:bg-indigo-500/20 text-indigo-600 dark:text-indigo-400 border-indigo-200 dark:border-indigo-500/30 shadow-2xs'
                                                : 'bg-white dark:bg-slate-800/50 text-slate-500 dark:text-slate-400 border-slate-200 dark:border-slate-700/50 hover:border-slate-300 dark:hover:border-slate-600'
                                            }`}
                                    >
                                        {s === 'all' ? 'All' : s}
                                        <span className="ml-1.5 bg-slate-100 dark:bg-slate-700/50 text-slate-600 dark:text-slate-300 px-1.5 py-0.5 rounded-full text-[10px]">
                                            {count}
                                        </span>
                                    </button>
                                );
            ...
            {subTab === 'loans' && (
                <div className="space-y-6">
                        ...
                        <div className="flex flex-wrap gap-2">
                            {['all', 'Pending', 'Active', 'Completed', 'Rejected'].map((s) => {
                                ...
                                return (
                                    ...
                                    >
                                        {s === 'all' ? 'All Loans' : s}
                                        <span className={`ml-1.5 px-1.5 py-0.5 rounded-full text-[10px] ${loanFilter === s ? 'bg-white/20 text-white' : 'bg-slate-100 dark:bg-slate-700/50 text-slate-600 dark:text-slate-300'}`}>
                                            {count}
                                        </span>
                                    </button>
                                );
                    ...
                    {filteredLoans.length === 0 ? (
                        ...
                        <div className="bg-white dark:bg-slate-800 rounded-3xl border border-slate-200 dark:border-slate-700/50 shadow-xs overflow-hidden">
                                        ...
                                        {filteredLoans.map((loan) => (
                                            <tr key={loan._id} className="hover:bg-slate-50/80 dark:hover:bg-slate-700/30 transition-colors">
                                                <td className="py-4 px-4">
                                                    <div className="flex items-center gap-3">
                                                        <div className="w-8 h-8 rounded-full bg-indigo-50 dark:bg-indigo-500/10 border border-indigo-100 dark:border-indigo-500/20 flex items-center justify-center font-bold text-indigo-600 dark:text-indigo-400 text-xs shrink-0">
                                                            {loan.employee?.name?.[0] || 'E'}
                                                        </div>
                                                        <div>
                                                            <p className="font-bold text-slate-800 dark:text-slate-100 leading-tight">{loan.employee?.name || 'Unknown'}</p>
                                                            <p className="text-[10px] text-slate-400 dark:text-slate-500">{loan.employee?.department || 'Department'}</p>
                                                        </div>
                                                    </div>
                                                </td>
                                                <td className="py-4 px-4 font-bold text-slate-800 dark:text-slate-200">{loan.purpose}</td>
                                                <td className="py-4 px-4 font-extrabold text-slate-900 dark:text-white">{formatPKR(loan.requestedAmount)}</td>
